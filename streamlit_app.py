...
with tab_coletar:
    ...
    meta_root_path = _resolve_path(DEFAULT_NWMP_RAW_ROOT)
    nwmp_sync_module = None
    nwmp_import_error = None
    try:
        ROOT = Path.cwd()
        if str(ROOT) not in sys.path:
            sys.path.insert(0, str(ROOT))
        import nwmp_sync as _nwmp_sync_module
    except Exception as exc:
        nwmp_import_error = exc
    else:
        nwmp_sync_module = _nwmp_sync_module

    if nwmp_sync_module is not None:
        try:
            last_sync_meta = nwmp_sync_module.load_last_sync_metadata(meta_root_path)
        except Exception:
            last_sync_meta = {}
    else:
        meta_candidate = (
            meta_root_path
            if meta_root_path.suffix.lower() == ".json"
            else meta_root_path / "last_sync_meta.json"
        )
        try:
            last_sync_meta = json.loads(meta_candidate.read_text(encoding="utf-8")) if meta_candidate.exists() else {}
        except Exception:
            last_sync_meta = {}
    ...
    if nwmp_sync_module is None:
        reason = nwmp_import_error or "motivo desconhecido"
        st.error(f"Falha ao importar nwmp_sync: {reason}")
    else:
        ...
        def _run_remote_sync():
            ...
            try:
                with st.spinner("Baixando, salvando RAW e atualizando histórico..."):
                    result = nwmp_sync_module.run_sync(
        ...
        def _run_local_sync():
            ...
            try:
                ...
                with st.spinner("Processando snapshot local e atualizando histórico..."):
                    result = nwmp_sync_module.run_sync_local_snapshot(
