...
def load_last_sync_metadata(
    raw_root: Union[str, Path],
    filename: str = "last_sync_meta.json",
) -> Dict[str, Any]:
    """Return persisted snapshot metadata for the given RAW root."""

    base_path = Path(raw_root)
    # If the caller passes the JSON file directly, honour it; otherwise assume a directory
    if base_path.suffix.lower() == ".json":
        meta_path = base_path
    else:
        meta_path = base_path / filename
    return _load_json_object(meta_path)
